...
const ProductosId = async ({params}: {params: {productosId: string}}) => {
  ...
  return (
    <main className='h-[1000px] lg:h-[750px] flex flex-col lg:flex-row'>
      ...
      <h2 className='mt-4 mb-4 ml-6'>Products/{product.name}</h2>
      <img className='w-[90%] lg:h-[480px] mx-auto' src={product.image} alt={product.name} />
      </div>

      <div className='ml-10 lg:w-[45%] mt-12'>
        <h1 className='text-center lg:font-bold lg:text-[40px]'>{product.name.toUpperCase()}</h1>
        <h3 className='lg:mt-10'>{product.description}</h3>
        <h3 className='lg:mt-10 font-bold lg:text-[30px]'>US${product.price}</h3>
        <div>
